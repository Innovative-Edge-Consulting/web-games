...
(function () {
  ...
  function normalizeLevelIndex(store){
    if (!store) return;
    if (!Number.isInteger(store.levelIndex)) store.levelIndex = 0;
    if (store.levelIndex < 0 || store.levelIndex >= LEVEL_LENGTHS.length) {
      store.levelIndex = 0;
    }
  }
  ...
  (async () => {
    ...
    async function startLevel(idx){
      ...
      // Hook Hint UX
if (window.WordscendUI) {
  window.WordscendUI.onHintCheck = function () {
    return canUseHintForLen(levelLen);
  };
  window.WordscendUI.onHintConsume = function () {
